...
type DogRow = {
  ...
  bloodline_label: string | null;
  pedigree_summary: string | null;
  photo_url: string | null;
  profile_notes: string | null;
  breeding_eligibility: string | null;
  proven_status: string | null;
  retirement_target: string | null;
...
type PairingRow = { id: string; organization_id: string; dam_id: string | null; sire_id: string | null; pairing_name: string | null; status: string; breeding_method: string | null; planned_breeding_start: string | null; planned_breeding_end: string | null; actual_breeding_date: string | null; projected_litter_size: string | null; projected_color_summary: string | null; projected_coat_summary: string | null; projected_price_range: string | null; breeder_goals: string | null; compatibility_summary: string | null; internal_score: number | null; notes: string | null; created_at: string; updated_at: string; };
type LitterRow = { id: string; organization_id: string; pairing_id: string | null; pregnancy_id: string | null; dam_id: string | null; sire_id: string | null; litter_name: string | null; status: string; bred_date: string | null; pregnancy_confirmed_at: string | null; due_date: string | null; whelp_date: string | null; expected_size: string | null; reservation_goal: number | null; notes: string | null; created_at: string; updated_at: string; };
...
export async function getBreederWorkspaceData(organizationId: string): Promise<BreederWorkspaceData> {
  ...
  const [dogsResult, healthResult, geneticResult, pairingsResult, littersResult, puppiesResult, tasksResult, eventsResult] = await Promise.all([
    supabase.from("breeding_dogs").select("*").eq("organization_id", organizationId).order("call_name", { ascending: true, nullsFirst: false }).returns<DogRow[]>(),
    ...
    supabase.from("breeding_pairings").select("*").eq("organization_id", organizationId).order("planned_breeding_start", { ascending: true, nullsFirst: false }).returns<PairingRow[]>(),
...
function mapDog(row: DogRow): Dog { const metadata = row.metadata ?? {}; return { id: row.id, organizationId: row.organization_id, callName: row.call_name ?? row.dog_name ?? "Unnamed Dog", registeredName: row.registered_name, role: row.role, status: row.status, dateOfBirth: row.date_of_birth, sex: row.sex, color: row.color, coat: row.coat, registry: row.registry, bloodline: row.bloodline_label, photoUrl: row.photo_url, notes: row.profile_notes ?? row.pedigree_summary, breedingEligibility: row.breeding_eligibility, provenStatus: row.proven_status, cycleNotes: typeof metadata.cycle_notes === "string" ? metadata.cycle_notes : null, createdAt: row.created_at, updatedAt: row.updated_at }; }
...
function mapPairing(row: PairingRow): BreedingPair { return { id: row.id, organizationId: row.organization_id, damId: row.dam_id, sireId: row.sire_id, pairingName: row.pairing_name, status: row.status, breedingMethod: row.breeding_method, plannedStart: row.planned_breeding_start, plannedEnd: row.planned_breeding_end, expectedLitterSize: row.projected_litter_size, colorCoatSummary: [row.projected_color_summary, row.projected_coat_summary].filter(Boolean).join(" • ") || null, reservationGoal: null, goals: row.breeder_goals, internalAnalysis: row.compatibility_summary, notes: row.notes, createdAt: row.created_at, updatedAt: row.updated_at }; }
function mapLitter(row: LitterRow): Litter { return { id: row.id, organizationId: row.organization_id, pairingId: row.pairing_id, damId: row.dam_id, sireId: row.sire_id, litterName: row.litter_name ?? "Unnamed Litter", status: row.status, breedingDate: row.bred_date, confirmationDate: row.pregnancy_confirmed_at, dueDate: row.due_date, whelpDate: row.whelp_date, expectedSize: row.expected_size, reservationGoal: row.reservation_goal, notes: row.notes, createdAt: row.created_at, updatedAt: row.updated_at }; }
function mapPuppy(row: PuppyRow): Puppy { return { id: row.id, organizationId: row.organization_id, litterId: row.litter_id, buyerId: row.buyer_id ?? null, puppyName: row.puppy_name, callName: row.call_name, dateOfBirth: row.date_of_birth, sex: row.sex, color: row.color, coat: row.coat, status: row.status, price: row.price, deposit: row.deposit, balance: row.balance, retained: Boolean(row.retained_for_program), publicVisible: Boolean(row.is_public), portalVisible: Boolean(row.portal_visible), goHomeReady: Boolean(row.go_home_ready), notes: row.notes, description: row.description, photoUrl: row.photo_url, createdAt: row.created_at, updatedAt: row.updated_at }; }
