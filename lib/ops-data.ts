...
export async function getPaymentsWorkspaceData() {
  ...
  const { data, error } = await admin
    .from("buyer_payments")
    .select("id,amount,payment_date,payment_type,payment_method,type,method,status,buyers(full_name)")
    .eq("organization_id", organizationId)
  ...
  return (data || []).filter((row: any) => row.buyers).map((row: any) => ({
    ...row,
    type: row.type ?? row.payment_type ?? "payment",
    method: row.method ?? row.payment_method ?? "manual",
  }));
}
