...
export default async function DashboardPage() {
  ...
  const lifecycle = [
    ...
    ["Buyers", "/dashboard/buyers", Users, "CRM + notes", live.counts.buyers],
  ...
  const healthCards = [
    ...
    ["Missing documents", String(live.counts.documentsPending), `${live.counts.applications} buyer applications pending`, ClipboardSignature, "blue"],
  ] as const;
          ...
          <div className="px-4 py-6 md:px-8 space-y-6">
            ...
            <div className="grid gap-6 xl:grid-cols-[1fr_380px]">
              ...
              <aside className="space-y-6">
                <Card><div className="flex items-center gap-3"><div className="relative flex h-12 w-12 items-center justify-center rounded-2xl bg-[#315842] text-white"><Bot className="h-6 w-6" /></div><div><p className="font-display text-2xl font-black">Chichi AI</p><p className="text-xs font-black uppercase tracking-[0.16em] text-[#7A6A55]">Data-aware assistant</p></div></div><div className="mt-5 rounded-[1.5rem] border border-[#DDD6C8] bg-[#FBFAF6] p-4"><p className="text-sm font-bold leading-7 text-[#52616B]">Ask Chichi about overdue balances, pending documents, buyer history, or create breeder reminders.</p></div><button className="mt-4 flex w-full items-center justify-center gap-2 rounded-2xl bg-[#315842] px-4 py-3 text-sm font-black text-white">Open Chichi<WandSparkles className="h-4 w-4" /></button></Card>
                <Card><SectionHeader eyebrow="Calendar" title="Upcoming" description="Live breeder_events from your workspace." />{(live.events.length ? live.events : [{ title: "No upcoming events", date: "Upcoming", tag: "Calendar" }]).map((event) => <div key={event.title} className="mt-4 rounded-3xl border border-[#DDD6C8] bg-[#FBFAF6] p-4"><div className="flex items-center gap-3"><CalendarClock className="h-5 w-5 text-[#B08A46]" /><div><p className="text-xs font-black uppercase tracking-[0.14em] text-[#B08A46]">{event.date} · {event.tag}</p><p className="text-sm font-black">{event.title}</p></div></div></div>)}</Card>
              </aside>
            </div>
          </div>
        </section>
